...
char* Codec::Serialize(const google::protobuf::Message *msg)
{
	/*
	 ...
	 |   total_len(include itself)    |   version info   |    name_len(with '\0' as end)   |   name    |    data    |
	 */
	...
	// total len
	*((int64_t*)p) = (int64_t)htonl(total_len);
	p += sizeof(int64_t);
	...
	// name len
	*((int32_t*)p) = (int32_t)htonl(type_name.size() + 1);
	p += sizeof(int32_t);
...
google::protobuf::Message* Codec::Deserialize(const char *bytes)
{
	...
	// data
	int size = total_len - sizeof(int64_t) - 16 - sizeof(int32_t) - name_len;
