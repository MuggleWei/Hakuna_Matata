...
static int on_row(void *data, int argc, char **argv, char **az_col_name)
{
	LOG_INFO("---------------------");
	for (int i = 0; i < argc; i++)
	{
		LOG_INFO("%s: %s = %s", (const char *)data, az_col_name[i], argv[i] ? argv[i] : "NULL");
	}
	return 0;
}
...
void query(sqlite3 *db)
{
	...
	const char *data = "Query callback";

	char *err_msg;
	int ret = sqlite3_exec(db, sql, on_row, (void*)data, &err_msg);
	if (ret != SQLITE_OK)
	{
