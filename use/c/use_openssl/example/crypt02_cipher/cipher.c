...
bool cipher_encrypt(const char *algorithm, const char *plaintext,
					...
					int key_size, int iv_size, int cipher_block_size,
...
int main(int argc, char *argv[])
{
	...
	size_t len = strlen(plaintext);
	unsigned int outbuf_len = len + EVP_MAX_BLOCK_LENGTH;
	unsigned char *outbuf = (unsigned char *)malloc(outbuf_len);
	...
	cipher_encrypt("AES-128-CBC", plaintext, aes128_key, aes128_iv,
				   AES_128_KEY_SIZE, AES_128_IV_SIZE, AES_128_BLOCK_SIZE,
				   outbuf, &outbuf_len);
	...
	cipher_encrypt("AES-256-CBC", plaintext, aes256_key, aes256_iv,
				   AES_256_KEY_SIZE, AES_256_IV_SIZE, AES_256_BLOCK_SIZE,
				   outbuf, &outbuf_len);
