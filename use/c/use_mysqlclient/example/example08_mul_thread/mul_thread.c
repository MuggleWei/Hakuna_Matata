...
muggle_thread_ret_t thread_routine(void *p_args)
{
	struct sys_args *args = (struct sys_args*)p_args;
	...
	if (mysql_real_connect(con, args->host, args->user, args->passwd, "testdb",
		...
		fprintf(stderr,
				"Usage: -h <host> -u <user> -p <password> -P <port>\n"
...
int main(int argc, char *argv[])
{
	...
	for (size_t i = 0; i < sizeof(threads) / sizeof(threads[0]); i++) {
		muggle_thread_create(&threads[i], thread_routine, &args);
	}
